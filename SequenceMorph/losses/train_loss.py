...
class VM_diffeo_loss(torch.nn.Module):
    ...
    def gradient_loss_mask(self, s, mask, penalty='l2'):
        # s is the deformation_matrix of shape (seq_length, channels=2, height, width)
        dy = torch.abs(s[:, :, 1:, :] - s[:, :, :-1, :])
        dx = torch.abs(s[:, :, :, 1:] - s[:, :, :, :-1])
    ...
    def mask_from_deformation_field(self, deformation, s=1):
        mag2 = torch.sum(deformation * deformation, dim=1)
        return torch.exp(-s*mag2)
...
class NCC(torch.nn.Module):
    ...
    def forward(self, I, J, mask=None):
        ...
        #cc = torch.clamp(cc0, 0.01, 0.99) # has nan for training the model
