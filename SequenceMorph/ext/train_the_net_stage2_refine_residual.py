...
import os, time
import math
from ME_nets.LagrangianMotionEstimationNet import Lagrangian_motion_estimate_net, Lagrangian_motion_residual_refinement_net
...
def train_Cardiac_Tagging_ME_net(net1, \
                                 net2, \
                                 np_data_root, \
                                 batch_size, \
                                 n_epochs, \
    ...
    net1.cuda()
    net1 = net1.float()
    ME_model = '/ailab/models/cardiac_ME/SequenceMorph/Stage1/S1_model.pth'
    net1.load_state_dict(torch.load(ME_model))
    # for net2
    net2.train()
    net2.cuda()
    net2 = net2.float()
    ...
    retrain_time_dict = []
    pretrained = False
    pretrained_model = '4_20.0551_model.pth'
    use_pretrained = True
    th = 20000
    while use_pretrained:
        if pretrained:
            net2.load_state_dict(torch.load(os.path.join(model_path, pretrained_model)))
        ...
        for outer_epoch in range(start_epoch, n_epochs):
            ...
            for combined_groupids in training_data_combined_groupids_list:
                ...
                for i, data in enumerate(training_set_loader):
                    ...
                    y = img[:, 0:34, ::]  # 1st frame also is the reference frame
                    ...
                    registered_cine1, registered_cine2, registered_cine_lag, flow_param, \
                    deformation_matrix, deformation_matrix_neg, deformation_matrix_lag = net1(y, x)

                    registered_cine1_lag2, lag_registered_cine2, registered_cine1_lag1_refined, flow_param2,  \
                    deformation_matrix2, deformation_matrix_neg2, refined_deformation_matrix_lag = net2(registered_cine_lag, x, deformation_matrix_lag, img[:, 0:1, ::])
                    ...
                    training_loss = kl_loss(x, flow_param2) + gama * recon_loss(x, registered_cine1_lag2) + \
                                     gama * recon_loss(registered_cine_lag, lag_registered_cine2) + beta_r * recon_loss(x, registered_cine1_lag1_refined)

                    training_loss.backward()
                    optimizer.step()
                    # statistic
                    epoch_loss_0 += training_loss.item()
                    if math.isnan(training_loss.item()) or training_loss.item() > th:
                        retrain_time_dict.append(time.asctime(time.localtime(time.time())))
                        np.savetxt(os.path.join(model_path, 'retrain_time.txt'), retrain_time_dict, fmt='%s')
                        break_flag = True
                        break
            ...
            for i, data in enumerate(val_set_loader):
                ...
                val_loss = kl_loss(x, val_flow_param2) + gama * recon_loss(x, val_registered_cine1_lag2) + \
                            gama * recon_loss(val_registered_cine1_lag1, lag_val_registered_cine2) + beta_r * recon_loss(x, val_registered_cine1_lag1_refined)
        ...
        torch.save(net2.state_dict(), os.path.join(model_path, 'S2_model.pth'))
...
if __name__ == '__main__':
    ...
    training_model_path = '/ailab/models/cardiac_ME/SequenceMorph/Stage2'
    ...
    net1 = Lagrangian_motion_estimate_net(vol_size, nf_enc, nf_dec)
    net2 = Lagrangian_motion_residual_refinement_net(vol_size, nf_enc, nf_dec)
    ...
    train_Cardiac_Tagging_ME_net(net1=net1,
                                 net2=net2,
