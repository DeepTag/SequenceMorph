...
class Lagrangian_flow_refinement(nn.Module):
    ...
    def forward(self, inf_flow, lag_flow, forward=True):
        """
        Pass input x through forward once
            :param inf_flow: inter frame motion field
        ...
        shape = inf_flow.shape
        seq_len = shape[0]
        refined_lag_flow = lag_flow
        for k in range(0, seq_len):
            if forward:
                src = lag_flow[k, ::].clone()
                sum_flow = inf_flow[k:k + 1, ::]
            else:
                src = inf_flow[k, ::]
...
class Lagrangian_motion_residual_refinement_net(nn.Module):
    """
    [Lagrangian_motion_refinement_net] is a class representing the architecture for Lagrangian motion refinement on a time
     sequence which is based on probabilistic diffeomoprhic VoxelMorph with a full sequence of lagrangian motion
     constraints. You may need to modify this code (e.g., number of layers) to suit your project needs.
    """
    ...
    def __init__(self, vol_size, enc_nf, dec_nf, test_phase=False, full_size=True, int_steps=7):
        ...
        Instiatiate lagrangian_motion_estimate_net model
            ...
            :param int_steps: the number of integration steps
        """
        super(Lagrangian_motion_residual_refinement_net, self).__init__()
        ...
        self.spatial_transform = SpatialTransformer(vol_size)
        self.lag_flow = Lagrangian_flow(vol_size)
        self.lag_flow_refine = Lagrangian_flow_refinement(vol_size)
        self.test_phase = test_phase

    def flow_estimate(self, net, src, tgt):
        """
        Pass input x through forward once
            :param src: moving image (warped reference image by the coarse Lagrangian flow) that we want to shift
            :param tgt: fixed image that we want to shift to
        """
        x = torch.cat([src, tgt], dim=1)
        x = net(x)
    ...
    def forward(self, lag_src, tgt, lag_flow, src_0):
        """
        Pass input x through forward once
            :param src: moving image (warped reference image by the coarse Lagrangian flow) that we want to shift
            :param tgt: fixed image that we want to shift to
            :param lag_flow: coarse Lagrangian flow estimated in the first step
        """

        lag_y_src, lag_y_tgt, flow_param, inf_flow, neg_inf_flow = self.flow_estimate(self.unet_model, lag_src, tgt)
        refined_lag_flow = self.lag_flow_refine(inf_flow, lag_flow)
        shape = lag_src.shape  # seq_length (batch_size), channel, height, width
        seq_length = shape[0]
        src_re = src_0.repeat(seq_length, 1, 1, 1)  # repeat the 1st frame to match other frames contained in a sequence
        lag_y_src_refined = self.spatial_transform(src_re, refined_lag_flow)
        return lag_y_src, lag_y_tgt, lag_y_src_refined, flow_param, inf_flow, neg_inf_flow, refined_lag_flow
...
class miccai2018_net_cc_san_grid_warp(nn.Module):
    """
    [miccai2018_net] is a class representing the architecture for probabilistic diffeomoprhic VoxelMorph presented in
    the MICCAI 2018 paper. You may need to modify this code (e.g., number of layers) to suit your project needs.

    The stationary velocity field (SVF) operates in a space (0.5)^3 of vol_size for computational reasons.
    """
